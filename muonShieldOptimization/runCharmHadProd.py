        ...
        for x in os.listdir(path + "/run" + str(run + n)):
            ...
            if not x.find("run_fixedTarget_" + str(orun + n)) < 0:
                if cycle == 0 and run == 0 and not x.find("1001") < 0:
                    continue
                if cycle == 0 and run == 0 and not x.find("1010") < 0:
                    continue
                cmd += path + "/run" + str(run + n) + "/" + x + "/" + fname.replace("XX", str(orun + n)) + " "
    if msel == "4":
        outFile = fname.replace("XX", "charm_" + str(orun) + "-" + str(orun + ncpus - 1))
    else:
        outFile = fname.replace("XX", "beauty_" + str(orun) + "-" + str(orun + ncpus - 1))
...
def compactifyCascade(run) -> None:
    ...
    if cmd.find("root") < 0:
        ...
        Npot = f.Get("2").GetBinContent(1) / 2.0 / chicc
        f.Close()
...
def statistics() -> None:
    ...
    for x in [0, 20, 40, 60, 80]:
        ...
        nPot += f.Get("2").GetBinContent(1) / 2.0
