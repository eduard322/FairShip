...
def configure_snd_siliconTarget(yaml_file, ship_geo):
    ...
    if ship_geo.SiliconTarget_geo.zPosition == "auto":
        ...
        z_shift = (ship_geo.mtc_geo.ironThick + ship_geo.mtc_geo.sciFiThick + ship_geo.mtc_geo.scintThick) * ship_geo.mtc_geo.nLayers / 2. + 132 / 2. + 10
        ship_geo.SiliconTarget_geo.zPosition = find_shield_center(ship_geo)[2][-1] - z_shift
...
def configure(run, ship_geo):
    ...
    if ship_geo.SND:
        ...
        elif 3 in getattr(ship_geo, 'SND_design', []):
            MuonShield.SetSNDSpace(
                hole = True,
                hole_dx = (ship_geo.mtc_geo.width + 5. * u.cm) / 2.,
                hole_dy = (ship_geo.mtc_geo.height + 5. * u.cm) / 2.
